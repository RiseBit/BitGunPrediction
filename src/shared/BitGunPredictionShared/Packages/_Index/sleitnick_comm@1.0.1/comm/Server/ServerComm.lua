...
-- Stephen Leitnick
-- December 20, 2021
-- Modified: Added built-in rate limiting support
...
-- Rate limit state: { [Player] = { [remoteName] = lastCallTime } }
...
local function createRateLimiter(remoteName: string, cooldownSeconds: number)
	return function(player: Player, _args: {any}): (boolean, ...any)
		local now = os.clock()
		
		if not rateLimitState[player] then
			rateLimitState[player] = {}
		end
		
		local lastCall = rateLimitState[player][remoteName] or 0
		
		if now - lastCall < cooldownSeconds then
			return false, nil
		end
		
		rateLimitState[player][remoteName] = now
		return true
	end
end
...
function ServerComm:BindFunction(
	...
	local middleware = inboundMiddleware or {}
	if rateLimit and rateLimit > 0 then
		table.insert(middleware, 1, createRateLimiter(name, rateLimit))
	end
	return Comm.BindFunction(self._instancesFolder, name, fn, middleware, outboundMiddleware)
end
...
function ServerComm:WrapMethod(
	...
	return Comm.WrapMethod(self._instancesFolder, tbl, name, middleware, outboundMiddleware)
end
...
function ServerComm:CreateSignal(
	...
	return Comm.CreateSignal(self._instancesFolder, name, unreliable, middleware, outboundMiddleware)
end
